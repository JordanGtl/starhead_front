...
import { Search, Crosshair, Zap, Target, SlidersHorizontal, X, Swords, Loader2 } from "lucide-react";
import { useTranslation } from "react-i18next";
import { fetchWeapons, type Weapon } from "@/data/weapons";
...
const WeaponStatCell = ({ labelKey, value }: { labelKey: string; value: string | number }) => {
  const { t } = useTranslation();
  return (
    <div className="rounded-md bg-secondary/80 px-2 py-1.5 text-center">
      <span className="block text-[10px] text-muted-foreground">{t(labelKey)}</span>
...
const WeaponCard = ({ weapon }: { weapon: Weapon }) => {
  ...
  const dps = weapon.dps ?? Math.round((weapon.damage ?? 0) * (weapon.rof ?? 0) / 60);
  const maxDps = 2500;
  const dpsPercent = Math.min((dps / maxDps) * 100, 100);
  const sizeLabel = weapon.size != null ? `S${weapon.size}` : "Personal";
    ...
    <Link to={`/weapons/${weapon.id}`} className="group relative flex flex-col overflow-hidden rounded-lg border border-border bg-card transition-all duration-300 hover:border-primary/40 hover:shadow-[0_0_24px_hsl(var(--primary)/0.1)]">
      ...
      <div className="flex flex-1 flex-col p-4">
        ...
        <div className="mt-auto grid grid-cols-3 gap-1.5 text-xs">
          <WeaponStatCell labelKey="weapons.alpha" value={weapon.damage ?? '—'} />
          <WeaponStatCell labelKey="weapons.rof" value={weapon.rof ?? '—'} />
          <WeaponStatCell labelKey="weapons.range" value={weapon.range != null ? `${weapon.range}m` : '—'} />
        </div>
...
const Weapons = () => {
  ...
  useEffect(() => {
    setLoading(true);
    fetchWeapons().then(setAllWeapons).finally(() => setLoading(false));
  }, []);

  const types = useMemo(() => [...new Set(allWeapons.map(w => w.type))].sort(), [allWeapons]);
  const sizes = useMemo(() => [...new Set(allWeapons.filter(w => w.size != null).map(w => String(w.size)))].sort(), [allWeapons]);
  const manufacturers = useMemo(() => [...new Set(allWeapons.map(w => w.manufacturer))].sort(), [allWeapons]);
  ...
  }, [allWeapons, search, selectedType, selectedSize, selectedManufacturer, selectedCategory]);

  const clearFilters = () => {
    setSelectedType(""); setSelectedSize(""); setSelectedManufacturer(""); setSelectedCategory(""); setSearch("");
  };

  const shipCount = useMemo(() => allWeapons.filter(w => w.category === "Ship").length, [allWeapons]);
  const fpsCount = useMemo(() => allWeapons.filter(w => w.category === "FPS").length, [allWeapons]);
      ...
      <div className="relative mb-8 overflow-hidden rounded-xl border border-border bg-gradient-to-br from-card via-card to-secondary p-8">
        ...
        <div className="relative">
          <div className="mb-2 flex items-center gap-2">
            ...
            <span className="text-xs font-semibold uppercase tracking-widest text-primary">{t("weapons.arsenal")}</span>
          </div>
          ...
          <div className="mt-6 flex flex-wrap gap-3">
            ...
            >
              ...
              <span className="font-medium">{t("weapons.all")}</span>
              <span className="rounded-full bg-secondary px-1.5 py-0.5 text-[10px] font-bold">{allWeapons.length}</span>
            </button>
            <button
            ...
            >
              ...
              <span className="font-medium">{t("weapons.shipMounted")}</span>
            ...
            >
              ...
              <span className="font-medium">{t("weapons.personal")}</span>
      ...
      <div className="mb-6 flex flex-wrap items-center gap-3">
        ...
        <button
          ...
          className={`inline-flex h-10 items-center gap-2 rounded-lg border px-4 text-sm font-medium transition-colors ${
            showFilters || activeFilterCount > 0 ? "border-primary/50 bg-primary/10 text-primary" : "border-border bg-card text-muted-foreground hover:text-foreground"
          }`}
        >
          ...
          <div className="grid gap-3 sm:grid-cols-3">
            ...
            <div>
              ...
              <select value={selectedSize} onChange={(e) => setSelectedSize(e.target.value)}
                ...
                {sizes.map((s) => <option key={s} value={s}>{t("common.size")} {s}</option>)}
              </select>
            </div>
            <div>
        ...
        <>
          <div className="mb-4">
            <p className="text-sm text-muted-foreground">
              <span className="font-semibold text-foreground">{filtered.length}</span> {t("weapons.found", { count: filtered.length })}
            </p>
          </div>
          ...
          {filtered.length === 0 && (
            <div className="flex flex-col items-center py-20 text-center">
              ...
              <button onClick={clearFilters} className="mt-4 text-sm text-primary hover:underline">{t("common.resetFilters")}</button>
            </div>
          )}
        </>
      )}
