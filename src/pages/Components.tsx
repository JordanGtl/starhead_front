...
import { Search, Shield, Zap, Thermometer, Gauge, Crosshair, SlidersHorizontal, X, Cpu, Loader2 } from "lucide-react";
import { useTranslation } from "react-i18next";
import { fetchComponents, type ShipComponent } from "@/data/components";
...
const gradeConfigBase: Record<string, { color: string; bar: string; labelKey: string }> = {
...
const ComponentCard = ({ c }: { c: ShipComponent }) => {
  ...
  const gradeBase = gradeConfigBase[c.grade];
  const grade = { ...gradeBase, label: t(gradeBase?.labelKey ?? "components.gradeD") };
...
const Components = () => {
  ...
  useEffect(() => {
    setLoading(true);
    fetchComponents().then(setAllComponents).finally(() => setLoading(false));
  }, []);

  const types = useMemo(() => [...new Set(allComponents.map((c) => c.type))], [allComponents]);
  const mfrs = useMemo(() => [...new Set(allComponents.map((c) => c.manufacturer))].sort(), [allComponents]);
  ...
  const typeStats = useMemo(() => types.map(t => ({
    type: t,
    count: allComponents.filter(c => c.type === t).length,
      ...
      <div className="relative mb-8 overflow-hidden rounded-xl border border-border bg-gradient-to-br from-card via-card to-secondary p-8">
        ...
        <div className="relative">
          <div className="mb-2 flex items-center gap-2">
            ...
            <span className="text-xs font-semibold uppercase tracking-widest text-primary">{t("components.equipment")}</span>
          </div>
        ...
        <>
          <div className="mb-4 flex items-center justify-between">
            <p className="text-sm text-muted-foreground">
              <span className="font-semibold text-foreground">{filtered.length}</span> {t("components.found", { count: filtered.length })}
            </p>
          </div>
