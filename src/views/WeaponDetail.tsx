...
const WeaponDetail = () => {
  ...
  useSEO({
    ...
    description: weapon ? `${weapon.name} — ${weapon.type} taille ${weapon.size}. Stats, DPS et portée dans StarHead.` : undefined,
    path: id ? `/weapons/${id}` : undefined,
  });
  ...
  if (!weapon) {
    return (
      <div className="container flex min-h-[60vh] flex-col items-center justify-center text-center">
        ...
        <Link href="/weapons" className="mt-4 text-primary hover:underline">← {t("weaponDetail.backToWeapons")}</Link>
      </div>
    );
  }
  ...
  return (
    <div className="container py-8">
      <Link href="/weapons" className="mb-6 inline-flex items-center gap-1.5 text-sm text-muted-foreground transition-colors hover:text-primary">
