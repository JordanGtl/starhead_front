...
const TYPE_STYLE: Record<string, {
  ...
  Armor:                  { bg: "from-violet-500/15 to-violet-600/5", border: "border-violet-500/30",   iconColor: "text-violet-400",  gradientFrom: "from-violet-500",  accent: "bg-violet-500",  tooltipBorder: "border-violet-500/50",  tooltipGlow: "shadow-violet-500/10",  nameColor: "text-violet-300",  Icon: Shield },
  Char_Armor_Helmet:      { bg: "from-blue-500/15 to-blue-600/5",     border: "border-blue-500/30",     iconColor: "text-blue-400",    gradientFrom: "from-blue-500",    accent: "bg-blue-500",    tooltipBorder: "border-blue-500/50",    tooltipGlow: "shadow-blue-500/10",    nameColor: "text-blue-300",    Icon: HardHat },
  Char_Armor_Torso:       { bg: "from-amber-500/15 to-amber-600/5",   border: "border-amber-500/30",    iconColor: "text-amber-400",   gradientFrom: "from-amber-500",   accent: "bg-amber-500",   tooltipBorder: "border-amber-500/50",   tooltipGlow: "shadow-amber-500/10",   nameColor: "text-amber-300",   Icon: Shirt },
  Char_Armor_Arms:        { bg: "from-cyan-500/15 to-cyan-600/5",     border: "border-cyan-500/30",     iconColor: "text-cyan-400",    gradientFrom: "from-cyan-500",    accent: "bg-cyan-500",    tooltipBorder: "border-cyan-500/50",    tooltipGlow: "shadow-cyan-500/10",    nameColor: "text-cyan-300",    Icon: Shield },
  Char_Armor_Legs:        { bg: "from-emerald-500/15 to-emerald-600/5",border:"border-emerald-500/30",  iconColor: "text-emerald-400", gradientFrom: "from-emerald-500", accent: "bg-emerald-500", tooltipBorder: "border-emerald-500/50", tooltipGlow: "shadow-emerald-500/10", nameColor: "text-emerald-300", Icon: PersonStanding },
  Char_Armor_Undersuit:   { bg: "from-teal-500/15 to-teal-600/5",     border: "border-teal-500/30",     iconColor: "text-teal-400",    gradientFrom: "from-teal-500",    accent: "bg-teal-500",    tooltipBorder: "border-teal-500/50",    tooltipGlow: "shadow-teal-500/10",    nameColor: "text-teal-300",    Icon: Shirt },
  Char_Armor_Backpack:    { bg: "from-orange-500/15 to-orange-600/5", border: "border-orange-500/30",   iconColor: "text-orange-400",  gradientFrom: "from-orange-500",  accent: "bg-orange-500",  tooltipBorder: "border-orange-500/50",  tooltipGlow: "shadow-orange-500/10",  nameColor: "text-orange-300",  Icon: Backpack },
  // Consommables
  FPS_Consumable:         { bg: "from-emerald-500/15 to-emerald-600/5", border: "border-emerald-500/30", iconColor: "text-emerald-400", gradientFrom: "from-emerald-500", accent: "bg-emerald-500", tooltipBorder: "border-emerald-500/50", tooltipGlow: "shadow-emerald-500/10", nameColor: "text-emerald-300", Icon: FlaskConical },
  Medicine:               { bg: "from-sky-500/15 to-sky-600/5",         border: "border-sky-500/30",     iconColor: "text-sky-400",     gradientFrom: "from-sky-500",     accent: "bg-sky-500",     tooltipBorder: "border-sky-500/50",     tooltipGlow: "shadow-sky-500/10",     nameColor: "text-sky-300",     Icon: Pill         },
  Food:                   { bg: "from-amber-500/15 to-amber-600/5",     border: "border-amber-500/30",   iconColor: "text-amber-400",   gradientFrom: "from-amber-500",   accent: "bg-amber-500",   tooltipBorder: "border-amber-500/50",   tooltipGlow: "shadow-amber-500/10",   nameColor: "text-amber-300",   Icon: Utensils     },
};
const DEFAULT_STYLE = {
...
const PersonalArmorStats = ({ d }: { d: PersonalArmorData }) => (
  <div>
    <StatRow icon={Shield}      label="Résistance impact"    value={fmt(d.impactForceResistance, 1)}                   accent="text-violet-400" />
    <StatRow icon={Snowflake}   label="Résistance froid min" value={fmt(d.minTempResistance, 1, " °C")}               accent="text-cyan-400" />
    <StatRow icon={Sun}         label="Résistance chaleur"   value={fmt(d.maxTempResistance, 1, " °C")}               accent="text-orange-400" />
    <StatRow icon={Activity}    label="Capacité radiation"   value={fmt(d.maxRadiationCapacity, 1)}                   accent="text-emerald-400" />
    <StatRow icon={Activity}    label="Dissipation radiation" value={fmt(d.radiationDissipationRate, 4, "/s")}         accent="text-emerald-400" />
    {d.signatures && d.signatures.length > 0 && (
      <>
        <Divider label="Signatures" />
        {d.signatures.map((sig, i) => {
          const s = sig as Record<string, unknown>;
          const name = (s.name ?? s.type ?? `Signal ${i + 1}`) as string;
          const val  = s.value != null ? fmt(s.value as number, 2) : null;
          return <StatRow key={i} icon={Radio} label={name} value={val} />;
        })}
      </>
    )}
  </div>
...
const WoWTooltip = ({ c }: { c: ShipComponent }) => {
  ...
  const statsContent = (() => {
    ...
    if (c.personalArmor)  return <PersonalArmorStats  d={c.personalArmor} />;
    return null;
  })();
...
// ─── Shops section (connectée à l'API) ───────────────────────────────────────
...
const ShopsSection = ({
  ...
  useEffect(() => {
    if (!itemRef || !versionId) return;
    setLoading(true);
    const qs = new URLSearchParams({ itemRef, gameVersion: String(versionId) });
    apiFetch<ShopEntry[]>(`/api/shop-inventory?${qs}`)
      .then(setShops)
      .catch(() => setShops([]))
  ...
  return (
    <div className="rounded-xl border border-border/50 bg-card/60 overflow-hidden">
      ...
      {!loading && shops.length > 0 && (
        <div className="divide-y divide-border/40">
          {shops.map((shop) => (
            <div key={shop.shopId} className="flex items-center gap-4 px-5 py-3.5 hover:bg-muted/10 transition-colors">
                ...
                {shop.buyPrice !== null && (
                  <div className="flex items-center gap-1 justify-end">
                    ...
                    <span className="text-sm font-bold text-amber-400 tabular-nums">
                      {shop.buyPrice.toLocaleString("fr-FR", { maximumFractionDigits: 0 })}
                    </span>
                    <span className="text-[10px] text-muted-foreground">aUEC</span>
                  </div>
                )}
                {shop.sellPrice !== null && (
                  <div className="flex items-center gap-1 justify-end">
                    <span className="text-[10px] text-emerald-400/70">Rachat :</span>
                    <span className="text-xs font-semibold text-emerald-400 tabular-nums">
                      {shop.sellPrice.toLocaleString("fr-FR", { maximumFractionDigits: 0 })}
                    </span>
                    <span className="text-[10px] text-muted-foreground">aUEC</span>
                  </div>
                )}
                {shop.buyPrice === null && shop.sellPrice === null && (
                  <span className="text-[11px] text-muted-foreground/50">Prix inconnu</span>
                )}
              </div>
            </div>
          ))}
...
const EquippedShipsSection = () => (
  <div className="rounded-xl border border-border/50 bg-card/60 overflow-hidden">
    ...
    <div className="flex flex-col items-center gap-2 py-10 text-center">
      ...
      <p className="text-sm text-muted-foreground/50">Disponible prochainement</p>
    </div>
  </div>
);
  ...
  if (error || !component) {
    return (
      <div className="container flex min-h-[60vh] flex-col items-center justify-center text-center">
        ...
        <Link href="/components" className="mt-4 text-sm text-primary hover:underline">← Retour</Link>
      </div>
    );
  }
