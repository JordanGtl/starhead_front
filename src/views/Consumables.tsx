...
import { Search, SlidersHorizontal, X, FlaskConical, Loader2, Pill, Utensils, Zap } from "lucide-react";
...
const TYPE_STYLE: Record<string, {
  ...
  FPS_Consumable: { bg: "from-emerald-500/15 to-emerald-600/5",  border: "border-emerald-500/50",  iconColor: "text-emerald-400",  nameColor: "text-emerald-300",  gradientFrom: "from-emerald-500",  Icon: FlaskConical },
  Medicine:       { bg: "from-sky-500/15 to-sky-600/5",          border: "border-sky-500/50",       iconColor: "text-sky-400",       nameColor: "text-sky-300",       gradientFrom: "from-sky-500",       Icon: Pill         },
  Food:           { bg: "from-amber-500/15 to-amber-600/5",      border: "border-amber-500/50",     iconColor: "text-amber-400",     nameColor: "text-amber-300",     gradientFrom: "from-amber-500",     Icon: Utensils     },
};
const DEFAULT_STYLE = { bg: "from-primary/15 to-primary/5", border: "border-primary/50", iconColor: "text-primary", nameColor: "text-primary", gradientFrom: "from-primary", Icon: FlaskConical };
...
const SUBTYPE_COLOR: Record<string, string> = {
  Drink:     "text-sky-400",
  Food:      "text-amber-400",
  Drug:      "text-violet-400",
  Medical:   "text-rose-400",
  Stimulant: "text-orange-400",
...
// ConsumableCard
// ---------------------------------------------------------------------------
const ConsumableCard = ({ item }: { item: ConsumableItem }) => {
  ...
  const subLabel = sub ? consumableSubtypeLabel(sub) : null;
    ...
    >
      ...
      <div className="px-4 pt-4 pb-3">
        ...
        <div className="mt-1 flex items-center gap-1.5 text-[11px] font-medium uppercase tracking-wider text-muted-foreground/70">
          <span>{consumableTypeLabel(item.type)}</span>
          {item.manufacturer && (
            <>
      ...
      <div className="px-4 py-3">
        <table className="w-full text-xs">
          <tbody>
            ...
            {subLabel && (
              <tr className="border-b border-border/30 last:border-0">
                <td className="py-1 text-muted-foreground">Catégorie</td>
...
const Consumables = () => {
  ...
  useSEO({ title: "Consommables", description: "Consommables, médicaments et nourriture dans Star Citizen.", path: "/consumables" });
  const { selectedVersion } = useVersion();

  const [allItems, setAllItems]               = useState<ConsumableItem[]>([]);
  ...
  useEffect(() => {
    ...
    setLoading(true);
    fetchConsumables({ gameVersion: selectedVersion.id, locale: i18n.language })
      .then(setAllItems)
      .catch(() => setAllItems([]))
  ...
  const types = useMemo(
    () => CONSUMABLE_TYPES.filter((t) => allItems.some((i) => i.type === t)),
    [allItems],
  );
  ...
  return (
    <div className="relative min-h-screen bg-background">
      <PageHeader
        breadcrumb={[{ label: t("nav.consumables"), icon: FlaskConical }]}
        label={t("nav.groupCraft")}
        labelIcon={FlaskConical}
        title={t("nav.consumables")}
        subtitle={t("nav.descConsumables")}
      />
          ...
          <div className="flex items-center gap-3">
            <div className="relative flex-1">
              ...
              <input
                ...
                placeholder="Rechercher un consommable ou un fabricant…"
            ...
            <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
              <div>
                ...
                <select value={selectedType} onChange={(e) => setSelectedType(e.target.value)}
                  ...
                  {types.map((tp) => <option key={tp} value={tp}>{consumableTypeLabel(tp)}</option>)}
                </select>
              </div>
              <div>
                ...
                <select value={selectedSubType} onChange={(e) => setSelectedSubType(e.target.value)}
                  className="h-9 w-full rounded-md border border-border bg-card px-3 text-sm text-foreground focus:border-primary focus:outline-none">
                  <option value="">{t("common.all")}</option>
                  {subTypes.map((s) => <option key={s} value={s}>{consumableSubtypeLabel(s) || s}</option>)}
                </select>
              </div>
              <div>
          ...
          <>
            <div className="mb-4">
              <p className="text-sm text-muted-foreground">
                {filtered.length} consommable{filtered.length !== 1 ? "s" : ""} trouvé{filtered.length !== 1 ? "s" : ""}
              </p>
            </div>
            <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
              {filtered.map((item) => <ConsumableCard key={item.id} item={item} />)}
            </div>
            {filtered.length === 0 && (
              <div className="flex flex-col items-center py-20 text-center">
                <FlaskConical className="mb-4 h-12 w-12 text-muted-foreground/30" />
                <p className="text-lg font-medium text-muted-foreground">Aucun consommable trouvé</p>
