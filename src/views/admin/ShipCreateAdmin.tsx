...
import {
  Rocket, ArrowLeft, Loader2, Check, AlertCircle, ImageIcon, Trash2, Upload,
} from 'lucide-react';
...
const MOVEMENT_CLASSES = [
  { value: 'Spaceship',     label: 'Vaisseau spatial (Spaceship)' },
  { value: 'ArcadeWheeled', label: 'Véhicule terrestre (ArcadeWheeled)' },
  { value: 'ArcadeHover',   label: 'Véhicule hoveur (ArcadeHover)' },
];
...
export default function ShipCreateAdmin() {
  ...
  const [movementClass, setMovementClass] = useState('Spaceship');
  ...
  // ── Fetch existing roles/careers ──
  useEffect(() => {
    if (!user?.roles?.includes('ROLE_ADMIN')) return;
    apiFetch<Filters>('/api/admin/ships/filters')
      .then(setFilters)
      .catch(() => {});
  }, [user]);
  ...
  const handleFile = (file: File) => {
    ...
    if (!allowed.includes(file.type)) {
      setImageError('Format non autorisé. Utilisez JPEG, PNG, WebP ou GIF.');
      return;
    }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (career.trim())       payload.career       = career.trim();
        ...
        <div className="relative z-10 flex min-h-[16vh] items-center">
          <div className="container pb-2 pt-8 flex items-end justify-between">
            <div>
              ...
              <h1 className="font-display text-4xl font-bold text-foreground">Nouveau vaisseau</h1>
      ...
      <div className="container max-w-3xl py-8">
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <section className="rounded-xl border border-border bg-card p-6 space-y-4">
            ...
            <div className="grid gap-4 sm:grid-cols-2">
              <div>
                ...
                <input
                  ...
                  placeholder="Ex. Aegis Avenger Titan"
                  required
              ...
              <div>
                ...
                <input
                  ...
                  placeholder="Ex. AEGS_Avenger_Titan"
                  className="w-full rounded-lg border border-border bg-background px-3 py-2.5 text-sm text-foreground placeholder:text-muted-foreground/40 focus:border-primary/50 focus:outline-none focus:ring-1 focus:ring-primary/30"
                />
              </div>
            </div>
            ...
            <div className="grid gap-4 sm:grid-cols-2">
              <div>
                <label className="mb-1.5 block text-xs font-semibold text-muted-foreground">Carrière</label>
                <select
                  value={career}
            ...
            <div>
              ...
              <textarea
                ...
                rows={3}
                placeholder="Description du vaisseau…"
